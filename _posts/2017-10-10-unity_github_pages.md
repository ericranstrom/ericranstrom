...
Step 5) Next we'll want to wire up the 'unity_dir' variable so jekyll will show the unity game you uploaded.   To do this, you'll need to update the layout used for your posts. If we look in `_config.yml` near the bottom, there is a section called 'defaults' where it specifies the default layout for posts, called 'single'.   This is the layout the posts will use, so in `_layouts/single.html` we'll make the change than handles the unity_dir variable.
